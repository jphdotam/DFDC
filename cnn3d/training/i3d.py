...
class InceptionI3d(nn.Module):
    ...
    def __init__(self, num_classes=400, spatial_squeeze=True, output_method='per_frame',
                 final_endpoint='Logits', name='inception_i3d', in_channels=3, dropout_keep_prob=0.5, input_res=224):
        ...
        assert output_method in ('per_frame', 'avg_pool', 'max_pool', 'dual_pool', 'raw')
        ...
        if self.input_res == 224:
            self.end_points[end_point] = Unit3D(in_channels=in_channels, output_channels=64, kernel_shape=[7, 7, 7],
                                                stride=(2, 2, 2), padding=(3, 3, 3), name=name + end_point)
        elif self.input_res == 112:
            self.end_points[end_point] = Unit3D(in_channels=in_channels, output_channels=64, kernel_shape=[7, 3, 3],
                                                stride=(2, 2, 2), padding=(3, 1, 1), name=name + end_point)
        elif self.input_res == '224hi':
            self.end_points[end_point] = Unit3D(in_channels=in_channels, output_channels=64, kernel_shape=[7, 5, 5],
                                                stride=(2, 2, 2), padding=(3, 1, 1), name=name + end_point)
        ...
        if self.input_res == 224:
            ...
            self.end_points[end_point] = MaxPool3dSamePadding(kernel_size=[1, 3, 3], stride=(1, 1, 1),
                                                              padding=0)
    ...
    def forward(self, x):
        ...
        # print(f"Got shape {x.shape} at logits")
        x = self.logits(self.dropout(self.avg_pool(x)))
        # print(f"Got shape {x.shape} from logits")
